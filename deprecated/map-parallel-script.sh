...
parallel --pipepart -a $filename awk -v mapq_threshold=${mapq_threshold} -f ${map_awk_script}

#awk -v filesuffix="test" -v mapq_threshold=$mapq_threshold -f ${map_awk_script} ${filename}
